...
function initContactForm(emailAddr) {
    ...
    const WEB3FORMS_ACCESS_KEY = '61a81f65-6d38-45bb-8aba-5154cf795b53';
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Honeypot \u2014 bots that fill this are ignored
        ...
        status.innerHTML = '<i class="bi bi-hourglass-split me-2"></i>Sending...';
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                status.innerHTML = '<i class="bi bi-check-circle me-2"></i>Message sent \u2014 I\'ll get back to you soon.';
                form.reset();
            } else {
                status.className = 'mt-3 text-center text-danger';
                const errMsg = (json && json.message) ? json.message : 'Something went wrong. Please try again or email me directly.';
                status.innerHTML = `<i class="bi bi-exclamation-circle me-2"></i>${errMsg}`;
            }
        } catch (err) {
            status.className = 'mt-3 text-center text-danger';
            status.innerHTML = `<i class="bi bi-exclamation-circle me-2"></i>Something went wrong. Please email <a href="mailto:${emailAddr}">${emailAddr}</a> instead.`;
        } finally {
